...
import com.xudod.attendance.emp_business_trip.domain.mapper.EmpBusinessTripMapper;
 ...
 */
@Service("empBusinessTripService")
public class EmpBusinessTripImp implements EmpBusinessTripService {
	...
	@Autowired
    private EmpBusinessTripMapper empBusinessTripMapper;
    
    private Logger logger= LoggerFactory.getLogger(EmpBusinessTripImp.class);
    
    /*代码分界head TODO*/

	
	@Override
	public Integer add(EmpBusinessTrip po) {
		try {
			...
			po.setSequence(empBusinessTripMapper.getSequence("emp_business_trip"));
			int res = empBusinessTripMapper.insert(po);
	...
	public Integer delbykey(String id) {
		try {
			...
			int res = empBusinessTripMapper.deleteById(id);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public EmpBusinessTrip getbykey(String id) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(id, "查询数据时，检测到查询id为null，查询失败！");
			EmpBusinessTrip po = empBusinessTripMapper.selectById(id);
			return po;
		} catch (Exception e) {
			throw e;
		}
	...
	@Override
	public IPage<EmpBusinessTrip> getpage(PageParameter<EmpBusinessTrip> page) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(page, "查询数据时，检测到分页对象为null，查询失败！");
			Page<EmpBusinessTrip> ipage = new Page<EmpBusinessTrip>();
			BeanUtils.copyProperties(page, ipage);
	        IPage<EmpBusinessTrip> orgIPage = empBusinessTripMapper.selectPage(ipage, new QueryWrapper<EmpBusinessTrip>());
	        return orgIPage;
		} catch (Exception e) {
			throw e;
		}
	...
	public Integer upStatebykey(UpStateVo upStateVo) {
		try {
			...
			int res = empBusinessTripMapper.upStatebykey(upStateVo.getId(), upStateVo.getStatus(), header);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public List<EmpBusinessTrip> getAll() {
		List<EmpBusinessTrip> res = empBusinessTripMapper.getAll();
		return res;
	}
