...
import com.xudod.attendance.att_confirm_rec.domain.mapper.AttConfirmRecMapper;
 ...
 */
@Service("attConfirmRecService")
public class AttConfirmRecImp implements AttConfirmRecService {
	...
	@Autowired
    private AttConfirmRecMapper attConfirmRecMapper;
    
    private Logger logger= LoggerFactory.getLogger(AttConfirmRecImp.class);
    
    /*代码分界head TODO*/

	
	@Override
	public Integer add(AttConfirmRec po) {
		try {
			...
			po.setSequence(attConfirmRecMapper.getSequence("att_confirm_rec"));
			int res = attConfirmRecMapper.insert(po);
	...
	public Integer delbykey(String id) {
		try {
			...
			int res = attConfirmRecMapper.deleteById(id);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public AttConfirmRec getbykey(String id) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(id, "查询数据时，检测到查询id为null，查询失败！");
			AttConfirmRec po = attConfirmRecMapper.selectById(id);
			return po;
		} catch (Exception e) {
			throw e;
		}
	...
	@Override
	public IPage<AttConfirmRec> getpage(PageParameter<AttConfirmRec> page) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(page, "查询数据时，检测到分页对象为null，查询失败！");
			Page<AttConfirmRec> ipage = new Page<AttConfirmRec>();
			BeanUtils.copyProperties(page, ipage);
	        IPage<AttConfirmRec> orgIPage = attConfirmRecMapper.selectPage(ipage, new QueryWrapper<AttConfirmRec>());
	        return orgIPage;
		} catch (Exception e) {
			throw e;
		}
	...
	public Integer upStatebykey(UpStateVo upStateVo) {
		try {
			...
			int res = attConfirmRecMapper.upStatebykey(upStateVo.getId(), upStateVo.getStatus(), header);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public List<AttConfirmRec> getAll() {
		List<AttConfirmRec> res = attConfirmRecMapper.getAll();
		return res;
	}
