...
import com.xudod.attendance.org_indeustry_relation.domain.mapper.OrgIndeustryRelationMapper;
 ...
 */
@Service("orgIndeustryRelationService")
public class OrgIndeustryRelationImp implements OrgIndeustryRelationService {
	...
	@Autowired
    private OrgIndeustryRelationMapper orgIndeustryRelationMapper;
    
    private Logger logger= LoggerFactory.getLogger(OrgIndeustryRelationImp.class);
    
    /*代码分界head TODO*/

	
	@Override
	public Integer add(OrgIndeustryRelation po) {
		try {
			...
			po.setSequence(orgIndeustryRelationMapper.getSequence("org_indeustry_relation"));
			int res = orgIndeustryRelationMapper.insert(po);
	...
	public Integer delbykey(String id) {
		try {
			...
			int res = orgIndeustryRelationMapper.deleteById(id);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public OrgIndeustryRelation getbykey(String id) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(id, "查询数据时，检测到查询id为null，查询失败！");
			OrgIndeustryRelation po = orgIndeustryRelationMapper.selectById(id);
			return po;
		} catch (Exception e) {
			throw e;
		}
	...
	@Override
	public IPage<OrgIndeustryRelation> getpage(PageParameter<OrgIndeustryRelation> page) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(page, "查询数据时，检测到分页对象为null，查询失败！");
			Page<OrgIndeustryRelation> ipage = new Page<OrgIndeustryRelation>();
			BeanUtils.copyProperties(page, ipage);
	        IPage<OrgIndeustryRelation> orgIPage = orgIndeustryRelationMapper.selectPage(ipage, new QueryWrapper<OrgIndeustryRelation>());
	        return orgIPage;
		} catch (Exception e) {
			throw e;
		}
	...
	public Integer upStatebykey(UpStateVo upStateVo) {
		try {
			...
			int res = orgIndeustryRelationMapper.upStatebykey(upStateVo.getId(), upStateVo.getStatus(), header);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public List<OrgIndeustryRelation> getAll() {
		List<OrgIndeustryRelation> res = orgIndeustryRelationMapper.getAll();
		return res;
	}
