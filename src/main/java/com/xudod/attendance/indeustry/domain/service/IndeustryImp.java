...
import com.xudod.attendance.indeustry.domain.mapper.IndeustryMapper;
 ...
 */
@Service("indeustryService")
public class IndeustryImp implements IndeustryService {
	...
	@Autowired
    private IndeustryMapper indeustryMapper;
    
    private Logger logger= LoggerFactory.getLogger(IndeustryImp.class);
    
    /*代码分界head TODO*/

	
	@Override
	public Integer add(Indeustry po) {
		try {
			...
			po.setSequence(indeustryMapper.getSequence("indeustry"));
			int res = indeustryMapper.insert(po);
	...
	public Integer delbykey(String id) {
		try {
			...
			int res = indeustryMapper.deleteById(id);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public Indeustry getbykey(String id) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(id, "查询数据时，检测到查询id为null，查询失败！");
			Indeustry po = indeustryMapper.selectById(id);
			return po;
		} catch (Exception e) {
			throw e;
		}
	...
	@Override
	public IPage<Indeustry> getpage(PageParameter<Indeustry> page) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(page, "查询数据时，检测到分页对象为null，查询失败！");
			Page<Indeustry> ipage = new Page<Indeustry>();
			BeanUtils.copyProperties(page, ipage);
	        IPage<Indeustry> orgIPage = indeustryMapper.selectPage(ipage, new QueryWrapper<Indeustry>());
	        return orgIPage;
		} catch (Exception e) {
			throw e;
		}
	...
	public Integer upStatebykey(UpStateVo upStateVo) {
		try {
			...
			int res = indeustryMapper.upStatebykey(upStateVo.getId(), upStateVo.getStatus(), header);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public List<Indeustry> getAll() {
		List<Indeustry> res = indeustryMapper.getAll();
		return res;
	}
