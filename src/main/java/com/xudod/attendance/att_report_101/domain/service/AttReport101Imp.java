...
import com.xudod.attendance.att_report_101.domain.mapper.AttReport101Mapper;
 ...
 */
@Service("attReport101Service")
public class AttReport101Imp implements AttReport101Service {
	...
	@Autowired
    private AttReport101Mapper attReport101Mapper;
    
    private Logger logger= LoggerFactory.getLogger(AttReport101Imp.class);
    
    /*代码分界head TODO*/

	
	@Override
	public Integer add(AttReport101 po) {
		try {
			...
			po.setSequence(attReport101Mapper.getSequence("att_report_101"));
			int res = attReport101Mapper.insert(po);
	...
	public Integer delbykey(String id) {
		try {
			...
			int res = attReport101Mapper.deleteById(id);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public AttReport101 getbykey(String id) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(id, "查询数据时，检测到查询id为null，查询失败！");
			AttReport101 po = attReport101Mapper.selectById(id);
			return po;
		} catch (Exception e) {
			throw e;
		}
	...
	@Override
	public IPage<AttReport101> getpage(PageParameter<AttReport101> page) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(page, "查询数据时，检测到分页对象为null，查询失败！");
			Page<AttReport101> ipage = new Page<AttReport101>();
			BeanUtils.copyProperties(page, ipage);
	        IPage<AttReport101> orgIPage = attReport101Mapper.selectPage(ipage, new QueryWrapper<AttReport101>());
	        return orgIPage;
		} catch (Exception e) {
			throw e;
		}
	...
	public Integer upStatebykey(UpStateVo upStateVo) {
		try {
			...
			int res = attReport101Mapper.upStatebykey(upStateVo.getId(), upStateVo.getStatus(), header);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public List<AttReport101> getAll() {
		List<AttReport101> res = attReport101Mapper.getAll();
		return res;
	}
