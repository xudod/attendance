...
import com.xudod.attendance.produce.domain.mapper.ProduceMapper;
...
public class ProduceImp implements ProduceService {
    ...
    private Logger logger= LoggerFactory.getLogger(ProduceImp.class);
    
    /*代码分界head TODO*/

	
	@Override
	public Integer add(Produce po) {
		try {
			...
			po.setSequence(produceMapper.getSequence("produce"));
			int res = produceMapper.insert(po);
	...
	public Integer delbykey(String id) {
		try {
			...
			int res = produceMapper.deleteById(id);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public Produce getbykey(String id) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(id, "查询数据时，检测到查询id为null，查询失败！");
			Produce po = produceMapper.selectById(id);
			return po;
		} catch (Exception e) {
			throw e;
		}
	...
	@Override
	public IPage<Produce> getpage(PageParameter<Produce> page) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(page, "查询数据时，检测到分页对象为null，查询失败！");
			Page<Produce> ipage = new Page<Produce>();
			BeanUtils.copyProperties(page, ipage);
	        IPage<Produce> orgIPage = produceMapper.selectPage(ipage, new QueryWrapper<Produce>());
	        return orgIPage;
		} catch (Exception e) {
			throw e;
		}
	...
	public Integer upStatebykey(UpStateVo upStateVo) {
		try {
			...
			int res = produceMapper.upStatebykey(upStateVo.getId(), upStateVo.getStatus(), header);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	public List<Produce> getAll() {
		List<Produce> res = produceMapper.getAll();
		return res;
	}
