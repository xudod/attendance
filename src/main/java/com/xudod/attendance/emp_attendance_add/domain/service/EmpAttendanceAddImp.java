...
import com.xudod.attendance.emp_attendance_add.domain.mapper.EmpAttendanceAddMapper;
 ...
 */
@Service("empAttendanceAddService")
public class EmpAttendanceAddImp implements EmpAttendanceAddService {
	...
	@Autowired
    private EmpAttendanceAddMapper empAttendanceAddMapper;
    
    private Logger logger= LoggerFactory.getLogger(EmpAttendanceAddImp.class);
    
    /*代码分界head TODO*/

	
	@Override
	public Integer add(EmpAttendanceAdd po) {
		try {
			...
			po.setSequence(empAttendanceAddMapper.getSequence("emp_attendance_add"));
			int res = empAttendanceAddMapper.insert(po);
	...
	public Integer delbykey(String id) {
		try {
			...
			int res = empAttendanceAddMapper.deleteById(id);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public EmpAttendanceAdd getbykey(String id) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(id, "查询数据时，检测到查询id为null，查询失败！");
			EmpAttendanceAdd po = empAttendanceAddMapper.selectById(id);
			return po;
		} catch (Exception e) {
			throw e;
		}
	...
	@Override
	public IPage<EmpAttendanceAdd> getpage(PageParameter<EmpAttendanceAdd> page) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(page, "查询数据时，检测到分页对象为null，查询失败！");
			Page<EmpAttendanceAdd> ipage = new Page<EmpAttendanceAdd>();
			BeanUtils.copyProperties(page, ipage);
	        IPage<EmpAttendanceAdd> orgIPage = empAttendanceAddMapper.selectPage(ipage, new QueryWrapper<EmpAttendanceAdd>());
	        return orgIPage;
		} catch (Exception e) {
			throw e;
		}
	...
	public Integer upStatebykey(UpStateVo upStateVo) {
		try {
			...
			int res = empAttendanceAddMapper.upStatebykey(upStateVo.getId(), upStateVo.getStatus(), header);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public List<EmpAttendanceAdd> getAll() {
		List<EmpAttendanceAdd> res = empAttendanceAddMapper.getAll();
		return res;
	}
