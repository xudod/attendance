...
import com.xudod.attendance.machine_info.domain.mapper.MachineInfoMapper;
...
public class MachineInfoImp implements MachineInfoService {
    ...
    private Logger logger= LoggerFactory.getLogger(MachineInfoImp.class);
    
    /*代码分界head TODO*/

	
	@Override
	public Integer add(MachineInfo po) {
		try {
			...
			po.setSequence(machineInfoMapper.getSequence("machine_info"));
			int res = machineInfoMapper.insert(po);
	...
	public Integer delbykey(String id) {
		try {
			...
			int res = machineInfoMapper.deleteById(id);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public MachineInfo getbykey(String id) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(id, "查询数据时，检测到查询id为null，查询失败！");
			MachineInfo po = machineInfoMapper.selectById(id);
			return po;
		} catch (Exception e) {
			throw e;
		}
	...
	@Override
	public IPage<MachineInfo> getpage(PageParameter<MachineInfo> page) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(page, "查询数据时，检测到分页对象为null，查询失败！");
			Page<MachineInfo> ipage = new Page<MachineInfo>();
			BeanUtils.copyProperties(page, ipage);
	        IPage<MachineInfo> orgIPage = machineInfoMapper.selectPage(ipage, new QueryWrapper<MachineInfo>());
	        return orgIPage;
		} catch (Exception e) {
			throw e;
		}
	...
	public Integer upStatebykey(UpStateVo upStateVo) {
		try {
			...
			int res = machineInfoMapper.upStatebykey(upStateVo.getId(), upStateVo.getStatus(), header);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	public List<MachineInfo> getAll() {
		List<MachineInfo> res = machineInfoMapper.getAll();
		return res;
	}
