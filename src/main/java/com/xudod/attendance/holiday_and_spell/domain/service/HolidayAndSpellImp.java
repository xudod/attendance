...
import com.xudod.attendance.holiday_and_spell.domain.mapper.HolidayAndSpellMapper;
 ...
 */
@Service("holidayAndSpellService")
public class HolidayAndSpellImp implements HolidayAndSpellService {
	...
	@Autowired
    private HolidayAndSpellMapper holidayAndSpellMapper;
    
    private Logger logger= LoggerFactory.getLogger(HolidayAndSpellImp.class);
    
    /*代码分界head TODO*/

	
	@Override
	public Integer add(HolidayAndSpell po) {
		try {
			...
			po.setSequence(holidayAndSpellMapper.getSequence("holiday_and_spell"));
			int res = holidayAndSpellMapper.insert(po);
	...
	public Integer delbykey(String id) {
		try {
			...
			int res = holidayAndSpellMapper.deleteById(id);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public HolidayAndSpell getbykey(String id) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(id, "查询数据时，检测到查询id为null，查询失败！");
			HolidayAndSpell po = holidayAndSpellMapper.selectById(id);
			return po;
		} catch (Exception e) {
			throw e;
		}
	...
	@Override
	public IPage<HolidayAndSpell> getpage(PageParameter<HolidayAndSpell> page) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(page, "查询数据时，检测到分页对象为null，查询失败！");
			Page<HolidayAndSpell> ipage = new Page<HolidayAndSpell>();
			BeanUtils.copyProperties(page, ipage);
	        IPage<HolidayAndSpell> orgIPage = holidayAndSpellMapper.selectPage(ipage, new QueryWrapper<HolidayAndSpell>());
	        return orgIPage;
		} catch (Exception e) {
			throw e;
		}
	...
	public Integer upStatebykey(UpStateVo upStateVo) {
		try {
			...
			int res = holidayAndSpellMapper.upStatebykey(upStateVo.getId(), upStateVo.getStatus(), header);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public List<HolidayAndSpell> getAll() {
		List<HolidayAndSpell> res = holidayAndSpellMapper.getAll();
		return res;
	}
