...
import com.xudod.attendance.all_punch_record.domain.mapper.AllPunchRecordMapper;
 ...
 */
@Service("allPunchRecordService")
public class AllPunchRecordImp implements AllPunchRecordService {
	...
	@Autowired
    private AllPunchRecordMapper allPunchRecordMapper;
    
    private Logger logger= LoggerFactory.getLogger(AllPunchRecordImp.class);
    
    /*代码分界head TODO*/

	
	@Override
	public Integer add(AllPunchRecord po) {
		try {
			...
			po.setSequence(allPunchRecordMapper.getSequence("all_punch_record"));
			int res = allPunchRecordMapper.insert(po);
	...
	public Integer delbykey(String id) {
		try {
			...
			int res = allPunchRecordMapper.deleteById(id);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public AllPunchRecord getbykey(String id) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(id, "查询数据时，检测到查询id为null，查询失败！");
			AllPunchRecord po = allPunchRecordMapper.selectById(id);
			return po;
		} catch (Exception e) {
			throw e;
		}
	...
	@Override
	public IPage<AllPunchRecord> getpage(PageParameter<AllPunchRecord> page) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(page, "查询数据时，检测到分页对象为null，查询失败！");
			Page<AllPunchRecord> ipage = new Page<AllPunchRecord>();
			BeanUtils.copyProperties(page, ipage);
	        IPage<AllPunchRecord> orgIPage = allPunchRecordMapper.selectPage(ipage, new QueryWrapper<AllPunchRecord>());
	        return orgIPage;
		} catch (Exception e) {
			throw e;
		}
	...
	public Integer upStatebykey(UpStateVo upStateVo) {
		try {
			...
			int res = allPunchRecordMapper.upStatebykey(upStateVo.getId(), upStateVo.getStatus(), header);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public List<AllPunchRecord> getAll() {
		List<AllPunchRecord> res = allPunchRecordMapper.getAll();
		return res;
	}
