...
import com.xudod.attendance.wifi_info.domain.mapper.WifiInfoMapper;
...
public class WifiInfoImp implements WifiInfoService {
    ...
    private Logger logger= LoggerFactory.getLogger(WifiInfoImp.class);
    
    /*代码分界head TODO*/

	
	@Override
	public Integer add(WifiInfo po) {
		try {
			...
			po.setSequence(wifiInfoMapper.getSequence("wifi_info"));
			int res = wifiInfoMapper.insert(po);
	...
	public Integer delbykey(String id) {
		try {
			...
			int res = wifiInfoMapper.deleteById(id);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public WifiInfo getbykey(String id) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(id, "查询数据时，检测到查询id为null，查询失败！");
			WifiInfo po = wifiInfoMapper.selectById(id);
			return po;
		} catch (Exception e) {
			throw e;
		}
	...
	@Override
	public IPage<WifiInfo> getpage(PageParameter<WifiInfo> page) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(page, "查询数据时，检测到分页对象为null，查询失败！");
			Page<WifiInfo> ipage = new Page<WifiInfo>();
			BeanUtils.copyProperties(page, ipage);
	        IPage<WifiInfo> orgIPage = wifiInfoMapper.selectPage(ipage, new QueryWrapper<WifiInfo>());
	        return orgIPage;
		} catch (Exception e) {
			throw e;
		}
	...
	public Integer upStatebykey(UpStateVo upStateVo) {
		try {
			...
			int res = wifiInfoMapper.upStatebykey(upStateVo.getId(), upStateVo.getStatus(), header);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	public List<WifiInfo> getAll() {
		List<WifiInfo> res = wifiInfoMapper.getAll();
		return res;
	}
