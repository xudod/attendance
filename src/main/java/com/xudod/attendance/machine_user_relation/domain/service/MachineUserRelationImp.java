...
import com.xudod.attendance.machine_user_relation.domain.mapper.MachineUserRelationMapper;
 ...
 */
@Service("machineUserRelationService")
public class MachineUserRelationImp implements MachineUserRelationService {
	...
	@Autowired
    private MachineUserRelationMapper machineUserRelationMapper;
    
    private Logger logger= LoggerFactory.getLogger(MachineUserRelationImp.class);
    
    /*代码分界head TODO*/

	
	@Override
	public Integer add(MachineUserRelation po) {
		try {
			...
			po.setSequence(machineUserRelationMapper.getSequence("machine_user_relation"));
			int res = machineUserRelationMapper.insert(po);
	...
	public Integer delbykey(String id) {
		try {
			...
			int res = machineUserRelationMapper.deleteById(id);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public MachineUserRelation getbykey(String id) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(id, "查询数据时，检测到查询id为null，查询失败！");
			MachineUserRelation po = machineUserRelationMapper.selectById(id);
			return po;
		} catch (Exception e) {
			throw e;
		}
	...
	@Override
	public IPage<MachineUserRelation> getpage(PageParameter<MachineUserRelation> page) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(page, "查询数据时，检测到分页对象为null，查询失败！");
			Page<MachineUserRelation> ipage = new Page<MachineUserRelation>();
			BeanUtils.copyProperties(page, ipage);
	        IPage<MachineUserRelation> orgIPage = machineUserRelationMapper.selectPage(ipage, new QueryWrapper<MachineUserRelation>());
	        return orgIPage;
		} catch (Exception e) {
			throw e;
		}
	...
	public Integer upStatebykey(UpStateVo upStateVo) {
		try {
			...
			int res = machineUserRelationMapper.upStatebykey(upStateVo.getId(), upStateVo.getStatus(), header);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public List<MachineUserRelation> getAll() {
		List<MachineUserRelation> res = machineUserRelationMapper.getAll();
		return res;
	}
