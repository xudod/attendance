...
import com.xudod.attendance.app_sysinfo_mdata.domain.mapper.AppSysinfoMdataMapper;
 ...
 */
@Service("appSysinfoMdataService")
public class AppSysinfoMdataImp implements AppSysinfoMdataService {
	...
	@Autowired
    private AppSysinfoMdataMapper appSysinfoMdataMapper;
    
    private Logger logger= LoggerFactory.getLogger(AppSysinfoMdataImp.class);
    
    /*代码分界head TODO*/

	
	@Override
	public Integer add(AppSysinfoMdata po) {
		try {
			...
			po.setSequence(appSysinfoMdataMapper.getSequence("app_sysinfo_mdata"));
			int res = appSysinfoMdataMapper.insert(po);
	...
	public Integer delbykey(String id) {
		try {
			...
			int res = appSysinfoMdataMapper.deleteById(id);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public AppSysinfoMdata getbykey(String id) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(id, "查询数据时，检测到查询id为null，查询失败！");
			AppSysinfoMdata po = appSysinfoMdataMapper.selectById(id);
			return po;
		} catch (Exception e) {
			throw e;
		}
	...
	@Override
	public IPage<AppSysinfoMdata> getpage(PageParameter<AppSysinfoMdata> page) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(page, "查询数据时，检测到分页对象为null，查询失败！");
			Page<AppSysinfoMdata> ipage = new Page<AppSysinfoMdata>();
			BeanUtils.copyProperties(page, ipage);
	        IPage<AppSysinfoMdata> orgIPage = appSysinfoMdataMapper.selectPage(ipage, new QueryWrapper<AppSysinfoMdata>());
	        return orgIPage;
		} catch (Exception e) {
			throw e;
		}
	...
	public Integer upStatebykey(UpStateVo upStateVo) {
		try {
			...
			int res = appSysinfoMdataMapper.upStatebykey(upStateVo.getId(), upStateVo.getStatus(), header);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public List<AppSysinfoMdata> getAll() {
		List<AppSysinfoMdata> res = appSysinfoMdataMapper.getAll();
		return res;
	}
