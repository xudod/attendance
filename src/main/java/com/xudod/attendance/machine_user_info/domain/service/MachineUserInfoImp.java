...
import com.xudod.attendance.machine_user_info.domain.mapper.MachineUserInfoMapper;
 ...
 */
@Service("machineUserInfoService")
public class MachineUserInfoImp implements MachineUserInfoService {
	...
	@Autowired
    private MachineUserInfoMapper machineUserInfoMapper;
    
    private Logger logger= LoggerFactory.getLogger(MachineUserInfoImp.class);
    
    /*代码分界head TODO*/

	
	@Override
	public Integer add(MachineUserInfo po) {
		try {
			...
			po.setSequence(machineUserInfoMapper.getSequence("machine_user_info"));
			int res = machineUserInfoMapper.insert(po);
	...
	public Integer delbykey(String id) {
		try {
			...
			int res = machineUserInfoMapper.deleteById(id);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public MachineUserInfo getbykey(String id) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(id, "查询数据时，检测到查询id为null，查询失败！");
			MachineUserInfo po = machineUserInfoMapper.selectById(id);
			return po;
		} catch (Exception e) {
			throw e;
		}
	...
	@Override
	public IPage<MachineUserInfo> getpage(PageParameter<MachineUserInfo> page) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(page, "查询数据时，检测到分页对象为null，查询失败！");
			Page<MachineUserInfo> ipage = new Page<MachineUserInfo>();
			BeanUtils.copyProperties(page, ipage);
	        IPage<MachineUserInfo> orgIPage = machineUserInfoMapper.selectPage(ipage, new QueryWrapper<MachineUserInfo>());
	        return orgIPage;
		} catch (Exception e) {
			throw e;
		}
	...
	public Integer upStatebykey(UpStateVo upStateVo) {
		try {
			...
			int res = machineUserInfoMapper.upStatebykey(upStateVo.getId(), upStateVo.getStatus(), header);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public List<MachineUserInfo> getAll() {
		List<MachineUserInfo> res = machineUserInfoMapper.getAll();
		return res;
	}
