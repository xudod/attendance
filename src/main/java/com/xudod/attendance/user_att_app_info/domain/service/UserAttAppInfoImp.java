...
import com.xudod.attendance.user_att_app_info.domain.mapper.UserAttAppInfoMapper;
 ...
 */
@Service("userAttAppInfoService")
public class UserAttAppInfoImp implements UserAttAppInfoService {
	...
	@Autowired
    private UserAttAppInfoMapper userAttAppInfoMapper;
    
    private Logger logger= LoggerFactory.getLogger(UserAttAppInfoImp.class);
    
    /*代码分界head TODO*/

	
	@Override
	public Integer add(UserAttAppInfo po) {
		try {
			...
			po.setSequence(userAttAppInfoMapper.getSequence("user_att_app_info"));
			int res = userAttAppInfoMapper.insert(po);
	...
	public Integer delbykey(String id) {
		try {
			...
			int res = userAttAppInfoMapper.deleteById(id);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public UserAttAppInfo getbykey(String id) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(id, "查询数据时，检测到查询id为null，查询失败！");
			UserAttAppInfo po = userAttAppInfoMapper.selectById(id);
			return po;
		} catch (Exception e) {
			throw e;
		}
	...
	@Override
	public IPage<UserAttAppInfo> getpage(PageParameter<UserAttAppInfo> page) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(page, "查询数据时，检测到分页对象为null，查询失败！");
			Page<UserAttAppInfo> ipage = new Page<UserAttAppInfo>();
			BeanUtils.copyProperties(page, ipage);
	        IPage<UserAttAppInfo> orgIPage = userAttAppInfoMapper.selectPage(ipage, new QueryWrapper<UserAttAppInfo>());
	        return orgIPage;
		} catch (Exception e) {
			throw e;
		}
	...
	public Integer upStatebykey(UpStateVo upStateVo) {
		try {
			...
			int res = userAttAppInfoMapper.upStatebykey(upStateVo.getId(), upStateVo.getStatus(), header);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public List<UserAttAppInfo> getAll() {
		List<UserAttAppInfo> res = userAttAppInfoMapper.getAll();
		return res;
	}
