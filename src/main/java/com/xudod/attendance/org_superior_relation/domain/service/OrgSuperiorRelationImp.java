...
import com.xudod.attendance.org_superior_relation.domain.mapper.OrgSuperiorRelationMapper;
 ...
 */
@Service("orgSuperiorRelationService")
public class OrgSuperiorRelationImp implements OrgSuperiorRelationService {
	...
	@Autowired
    private OrgSuperiorRelationMapper orgSuperiorRelationMapper;
    
    private Logger logger= LoggerFactory.getLogger(OrgSuperiorRelationImp.class);
    
    /*代码分界head TODO*/

	
	@Override
	public Integer add(OrgSuperiorRelation po) {
		try {
			...
			po.setSequence(orgSuperiorRelationMapper.getSequence("org_superior_relation"));
			int res = orgSuperiorRelationMapper.insert(po);
	...
	public Integer delbykey(String id) {
		try {
			...
			int res = orgSuperiorRelationMapper.deleteById(id);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public OrgSuperiorRelation getbykey(String id) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(id, "查询数据时，检测到查询id为null，查询失败！");
			OrgSuperiorRelation po = orgSuperiorRelationMapper.selectById(id);
			return po;
		} catch (Exception e) {
			throw e;
		}
	...
	@Override
	public IPage<OrgSuperiorRelation> getpage(PageParameter<OrgSuperiorRelation> page) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(page, "查询数据时，检测到分页对象为null，查询失败！");
			Page<OrgSuperiorRelation> ipage = new Page<OrgSuperiorRelation>();
			BeanUtils.copyProperties(page, ipage);
	        IPage<OrgSuperiorRelation> orgIPage = orgSuperiorRelationMapper.selectPage(ipage, new QueryWrapper<OrgSuperiorRelation>());
	        return orgIPage;
		} catch (Exception e) {
			throw e;
		}
	...
	public Integer upStatebykey(UpStateVo upStateVo) {
		try {
			...
			int res = orgSuperiorRelationMapper.upStatebykey(upStateVo.getId(), upStateVo.getStatus(), header);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public List<OrgSuperiorRelation> getAll() {
		List<OrgSuperiorRelation> res = orgSuperiorRelationMapper.getAll();
		return res;
	}
