...
import com.xudod.attendance.user_schedule_change.domain.mapper.UserScheduleChangeMapper;
 ...
 */
@Service("userScheduleChangeService")
public class UserScheduleChangeImp implements UserScheduleChangeService {
	...
	@Autowired
    private UserScheduleChangeMapper userScheduleChangeMapper;
    
    private Logger logger= LoggerFactory.getLogger(UserScheduleChangeImp.class);
    
    /*代码分界head TODO*/

	
	@Override
	public Integer add(UserScheduleChange po) {
		try {
			...
			po.setSequence(userScheduleChangeMapper.getSequence("user_schedule_change"));
			int res = userScheduleChangeMapper.insert(po);
	...
	public Integer delbykey(String id) {
		try {
			...
			int res = userScheduleChangeMapper.deleteById(id);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public UserScheduleChange getbykey(String id) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(id, "查询数据时，检测到查询id为null，查询失败！");
			UserScheduleChange po = userScheduleChangeMapper.selectById(id);
			return po;
		} catch (Exception e) {
			throw e;
		}
	...
	@Override
	public IPage<UserScheduleChange> getpage(PageParameter<UserScheduleChange> page) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(page, "查询数据时，检测到分页对象为null，查询失败！");
			Page<UserScheduleChange> ipage = new Page<UserScheduleChange>();
			BeanUtils.copyProperties(page, ipage);
	        IPage<UserScheduleChange> orgIPage = userScheduleChangeMapper.selectPage(ipage, new QueryWrapper<UserScheduleChange>());
	        return orgIPage;
		} catch (Exception e) {
			throw e;
		}
	...
	public Integer upStatebykey(UpStateVo upStateVo) {
		try {
			...
			int res = userScheduleChangeMapper.upStatebykey(upStateVo.getId(), upStateVo.getStatus(), header);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public List<UserScheduleChange> getAll() {
		List<UserScheduleChange> res = userScheduleChangeMapper.getAll();
		return res;
	}
