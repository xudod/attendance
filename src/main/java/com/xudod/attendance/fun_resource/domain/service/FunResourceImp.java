...
import com.xudod.attendance.fun_resource.domain.mapper.FunResourceMapper;
...
public class FunResourceImp implements FunResourceService {
	...
	public Integer upStatebykey(UpStateVo upStateVo) {
		try {
			...
			int res = funResourceMapper.upStatebykey(upStateVo.getId(), upStateVo.getStatus(), header);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
