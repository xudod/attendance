...
import com.xudod.attendance.user_org_relation.domain.mapper.UserOrgRelationMapper;
...
public class UserOrgRelationImp implements UserOrgRelationService {
    ...
    private Logger logger= LoggerFactory.getLogger(UserOrgRelationImp.class);
    
    /*代码分界head TODO*/

	
	@Override
	public Integer add(UserOrgRelation po) {
		try {
			...
			po.setSequence(userOrgRelationMapper.getSequence("user_org_relation"));
			int res = userOrgRelationMapper.insert(po);
	...
	public Integer delbykey(String id) {
		try {
			...
			int res = userOrgRelationMapper.deleteById(id);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public UserOrgRelation getbykey(String id) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(id, "查询数据时，检测到查询id为null，查询失败！");
			UserOrgRelation po = userOrgRelationMapper.selectById(id);
			return po;
		} catch (Exception e) {
			throw e;
		}
	...
	@Override
	public IPage<UserOrgRelation> getpage(PageParameter<UserOrgRelation> page) {
		try {
			DataCorrectCheck.PojoIsNotNullCheck(page, "查询数据时，检测到分页对象为null，查询失败！");
			Page<UserOrgRelation> ipage = new Page<UserOrgRelation>();
			BeanUtils.copyProperties(page, ipage);
	        IPage<UserOrgRelation> orgIPage = userOrgRelationMapper.selectPage(ipage, new QueryWrapper<UserOrgRelation>());
	        return orgIPage;
		} catch (Exception e) {
			throw e;
		}
	...
	public Integer upStatebykey(UpStateVo upStateVo) {
		try {
			...
			int res = userOrgRelationMapper.upStatebykey(upStateVo.getId(), upStateVo.getStatus(), header);
			addUpDelResCheck(res, "更新数据失败！");
			return res;
		} catch (Exception e) {
			throw e;
	...
	@Override
	public List<UserOrgRelation> getAll() {
		List<UserOrgRelation> res = userOrgRelationMapper.getAll();
		return res;
	}
